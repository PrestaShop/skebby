...
class Skebby extends Module
{
    ...
    public function uninstall()
    {
        ...
        $success = ($this->removeConfigKeys() && $this->hookUninstall() && parent::uninstall());
    ...
    {
        return Configuration::get('SKEBBY_ORDER_NOTIFICATION_ACTIVE') == 1
        && Configuration::get('SKEBBY_ORDER_TEMPLATE') != '';
    }
    ...
    {
        return Configuration::get('SKEBBY_ALPHASENDER_ACTIVE') == 1
        && Configuration::get('SKEBBY_DEFAULT_ALPHASENDER') != '';
    }
    ...
    private function populateOrderVariables($order, $address)
    {
        ...
        $customer_civility_result = Db::getInstance()->executeS('SELECT id_gender,firstname,lastname
        ...
        if (isset($customer_civility_result[0])) {
            if (Tools::strtolower($firstname) === Tools::strtolower($customer_civility_result[0]['firstname']) &&
                ...
                $civility_value = (isset($customer_civility_result['0']['id_gender'])) ?
                    $customer_civility_result['0']['id_gender'] : '';
            }
        }
    ...
    public function displayForm()
    {
        ...
        $data['this_path'] = $this->_path;
        $data['base_url'] = Tools::getHttpHost(true, true).__PS_BASE_URI__.'modules/skebby/';
        ...
        $helper->fields_value['SKEBBY_ALPHASENDER_ACTIVE'] =
            ((string)Configuration::get('SKEBBY_ALPHASENDER_ACTIVE') == '1');
        $helper->fields_value['SKEBBY_ORDER_NOTIFICATION_ACTIVE'] =
            ((string)Configuration::get('SKEBBY_ORDER_NOTIFICATION_ACTIVE') == '1');
        ...
        $helper->fields_value['SKEBBY_SHIPMENTSTATUS_NOTIFICATION_TEMPLATE'] =
            Configuration::get('SKEBBY_SHIPMENTSTATUS_NOTIFICATION_TEMPLATE');
        $helper->fields_value['SKEBBY_SHIPMENTSTATUS_NOTIFICATION_ACTIVE'] =
            ((string)Configuration::get('SKEBBY_SHIPMENTSTATUS_NOTIFICATION_ACTIVE') ==
            '1');
...
// 		$theform .= '</div>';
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit'.$this->name)) {
            ...
            if ($use_alpha_sender) {
                ...
                if (!$skebby_alpha_sender || empty($skebby_alpha_sender)
                    || !$this->isValidAlphasender($skebby_alpha_sender)) {
            ...
            if (!$skebby_mobile_number || empty($skebby_mobile_number)
                || !$this->isValidMobileNumber($skebby_mobile_number)) {
            ...
            if (!$skebby_default_quality || empty($skebby_default_quality)
            ...
            if ($skebby_neworder_active) {
                ...
                if (!$skebby_order_recipient || empty($skebby_order_recipient)
                    || !Validate::isGenericName($skebby_order_recipient) ||
            ...
            if ($skebby_shipment_active) {
                ...
                if (!$skebby_shipment_template || empty($skebby_shipment_template)
                    || !Validate::isGenericName($skebby_shipment_template)) {
    ...
    public function displayViewCustomerLink($id, $token = null, $name = null)
    {
        $this->smarty->assign(array(
            'href' => 'index.php?controller=AdminCustomers&id_customer='
                .(int)$id.'&sync&token='.Tools::getAdminTokenLite('AdminCustomers'),
            'action' => $this->l('View'),
            'disable' => !((int)$id > 0),
        ));
    ...
    {
        $dbquery = new DbQuery();
        $dbquery->select('c.`id_customer` AS `id`, CONCAT("+", CONCAT(p.`call_prefix`,a.`phone_mobile` ))
        AS `phone_mobile`, s.`name` AS `shop_name`, gl.`name` AS `gender`, c.`lastname`, c.`firstname`, c.`email`,
        ...
        $dbquery->leftJoin('gender_lang', 'gl', 'g.id_gender = gl.id_gender AND
        gl.id_lang = '.(int)$this->context->employee->id_lang);
